...
def render_logo(kind: str = "hero", max_width_px: int = HERO_LOGO_W, center: bool = True, margin: str = "0.5rem 0"):
    ...
    container_style = f"display:flex;{'justify-content:center;' if center else ''}margin:{margin};"
    if not img:
        st.markdown(
            f"""
            <div style="{container_style}">
              <div style="color:#94a3b8;border:1px dashed #334155;border-radius:8px;padding:12px 16px;background:rgba(15,23,42,.4)">
                Missing logo file: <code>{img_path}</code>
              </div>
            </div>
            """,
    ...
    st.markdown(
        f"""
        <div class="logo-wrap" style="{container_style}">
          <img src="data:image/png;base64,{_img_to_base64(img)}"
               alt="AI DataChat"
               style="display:block;width:100%;max-width:{max_width_px}px;height:auto;image-rendering:-webkit-optimize-contrast"/>
        </div>
        """,
        unsafe_allow_html=True
    )
...
# Utility
# =========================================================
def detect_sales_dataset(df: pd.DataFrame) -> bool:
    if df is None:
        return False
    sales_keywords = [
...
def sales_pipeline_analyzer():
    ...
    # Auto-detect
    stage_col = amount_col = date_col = owner_col = None
    for col in df.columns:
        col_lower = col.lower()
        if 'stage' in col_lower and stage_col is None:
            stage_col = col
        elif any(k in col_lower for k in ['amount', 'value', 'revenue']) and amount_col is None:
            amount_col = col
        elif any(k in col_lower for k in ['close', 'date']) and date_col is None:
            date_col = col
        elif any(k in col_lower for k in ['owner', 'rep', 'sales']) and owner_col is None:
            owner_col = col
    ...
    with col1:
        ...
        numeric_cols_for_amount = df.select_dtypes(include=[np.number]).columns.tolist()
        amount_column = st.selectbox(
            "Deal Amount Column:",
            numeric_cols_for_amount if numeric_cols_for_amount else df.columns.tolist(),
            index=(numeric_cols_for_amount.index(amount_col) if (amount_col in numeric_cols_for_amount) else 0) if numeric_cols_for_amount else 0,
    ...
    if st.button("📊 Generate Pipeline Analysis", type="primary", key="generate_sales_analysis"):
        with st.spinner("Analyzing sales pipeline..."):
            try:
                ...
                col_m1.metric("Total Deals", f"{total_deals:,}")
                col_m2.metric("Total Pipeline Value", f"${total_value:,.0f}")
                col_m3.metric("Closed Value", f"${closed_value:,.0f}")
                ...
                with col_viz2:
                    ...
                    if score >= 80:
                        color = "#10b981"; grade = "Excellent"
                    elif score >= 60:
                        color = "#f59e0b"; grade = "Good"
                    else:
                ...
                if owner_column != "None":
                    ...
                    fig_owner = px.bar(
                        owner_summary.head(10), x='Owner', y='Total Value',
                        title="Top 10 Performers by Pipeline Value", color='Total Value',
                ...
                insights = []
                if win_rate < 15:
                    insights.append(f"⚠️ Win rate is {win_rate:.1f}%, below typical 15–20%. Revisit qualification criteria.")
                elif win_rate > 25:
                    insights.append(f"✅ Win rate of {win_rate:.1f}% is excellent.")
                if len(stage_list) >= 2:
                    top_stage_count = stage_summary.iloc[0]['Deal Count']
                    bottom_stage_count = stage_summary.iloc[-1]['Deal Count']
                    drop_rate = ((top_stage_count - bottom_stage_count) / max(top_stage_count, 1) * 100)
                    if drop_rate > 70:
                        insights.append(f"⚠️ High drop-off rate of {drop_rate:.1f}% across the pipeline.")
                if score < 70:
                    insights.append(f"⚠️ Data quality score {score}/100 may reduce forecast accuracy.")
                ...
                export_data = {'Stage Summary': stage_summary, 'Conversion Analysis': conversion_df}
                if owner_column != "None":
...
def clean_data_interface():
    ...
    with col1:
        ...
        missing_action = st.selectbox(
            ...
            ["Keep as is", "Drop rows with any missing", "Drop rows with all missing",
...
def create_visualizations(df: pd.DataFrame):
    ...
    if viz_type == "Histogram":
        if numeric_cols:
            ...
            col3.metric("Std Dev", f"{df[col].std():.2f}")
        ...
        if cat_cols and numeric_cols:
            ...
            agg_func = st.selectbox("Aggregation:", agg_options, index=agg_options.index(default_agg), key="bar_agg")
...
def display_data_overview():
    ...
    col3.metric("Memory", f"{memory_mb:.2f} MB")
        ...
        if categorical_stats:
            for col, stats in categorical_stats.items():
                with st.expander(f"{col}"):
                    ...
                    if stats['value_counts'] is not None:
...
def forecasting_interface():
    ...
    col1, col2 = st.columns(2)
    with col1:
        date_col = st.selectbox("Select Date Column:", time_cols, key="forecast_date_col")
    with col2:
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        if not numeric_cols:
    ...
    if st.button("Generate 6-Month Forecast", key="generate_forecast_button"):
        with st.spinner("Preparing time series data..."):
            try:
                ...
                col_a, col_b, col_c = st.columns(3)
                col_a.metric("Total Data Points", validation['total_points'])
                col_b.metric("Frequency", validation['frequency'])
        ...
        with st.spinner("Generating forecast..."):
            try:
                ...
                metric_col3.metric("MAPE", f"{metrics['mape']:.1f}%")
                metric_col4.metric("R²", f"{metrics['r_squared']:.3f}")
                st.markdown("---")
                st.subheader("Forecast Visualization")
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=series.index, y=series.values, mode='lines', name='Historical Data',
                                         line=dict(color='#06b6d4', width=2)))
                fig.add_trace(go.Scatter(x=fitted.index, y=fitted.values, mode='lines', name='Fitted Values',
                                         line=dict(color='#f59e0b', dash='dash', width=2)))
                fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df['forecast'].values, mode='lines',
                                         name='6-Month Forecast', line=dict(color='#ef4444', width=3)))
                fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df['upper_bound'].values,
                                         mode='lines', line=dict(width=0), showlegend=False, hoverinfo='skip'))
                fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df['lower_bound'].values,
                                         mode='lines', line=dict(width=0), fill='tonexty',
                                         fillcolor='rgba(239, 68, 68, 0.2)', name='95% Confidence'))
                fig.update_layout(
                ...
                col1, col2, col3 = st.columns(3)
                col1.metric("Current Value", f"{last_actual:.2f}")
                col2.metric("6-Month Forecast", f"{last_forecast:.2f}")
                col3.metric("Expected Change", f"{percent_change:+.1f}%")
                st.markdown("---")
...
def scenario_simulation_interface():
    ...
    input_method = st.radio("Input Method:", ["Text Description", "Manual Selection"], key="scenario_input_method")
    changes = []
    if input_method == "Text Description":
        ...
        scenario_text = st.text_input("Scenario:", placeholder="increase revenue by 20%", key="scenario_text_input")
        if scenario_text:
        ...
        for i in range(num_changes):
            ...
            with col1:
                variable = st.selectbox(f"Variable {i+1}:", numeric_cols, key=f"var_select_{i}")
            with col2:
                change_pct = st.number_input("Change %:", value=10.0, step=5.0, key=f"change_pct_{i}")
            changes.append({'variable': variable, 'change_percent': change_pct})
    corr_threshold = st.slider("Correlation threshold:", 0.1, 0.9, 0.3, 0.1, key="corr_threshold")
    if st.button("Run Simulation", key="run_simulation_button") and changes:
        with st.spinner("Simulating scenario..."):
            ...
            for change in results['primary_changes']:
                col1, col2, col3 = st.columns(3)
                col1.metric(change['variable'], f"{change['new_value']:.2f}", f"{change['change_percent']:+.1f}%")
                col2.write(f"**Current:** {change['current_value']:.2f}")
                col2.write(f"**New:** {change['new_value']:.2f}")
                col3.write("**Change:**")
                col3.write(f"{change['absolute_change']:+.2f}")
            st.markdown("---")
            if results['secondary_effects']:
                ...
                for effect in results['secondary_effects']:
                    with st.expander(f"{effect['variable']} (corr: {effect['correlation']:.3f})"):
                        ...
                        col_a.metric("Estimated New Value", f"{effect['estimated_value']:.2f}",
                                     f"{effect['estimated_change_percent']:+.1f}%")
                        col_b.write(f"**Current:** {effect['current_value']:.2f}")
            ...
            with st.spinner("Thinking..."):
                try:
                    response = st.session_state.ai_agent.analyze_data(
                        st.session_state.df_filtered, prompt, st.session_state.data_summary
                    )
                    st.write(response)
    ...
    if st.session_state.df is None:
        ...
        if st.session_state.is_sales_data:
            tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
                "📊 Overview", "📈 Statistics", "💰 Sales Pipeline",
                "⏰ Forecasting", "🎯 Scenarios", "💬 Chat", "💳 Subscription"
            ])
            with tab3:
